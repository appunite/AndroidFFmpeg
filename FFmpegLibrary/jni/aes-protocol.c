...
static int aes_open(URLContext *h, const char *uri, int flags)
{
    ...
    log_hex("aes_open: raw_key: %s", c->key, RAW_KEY_SIZE);
    ...
    log_hex("aes_open: sha256_key: %s", sha256_key, SHA256_KEY_SIZE);

    unsigned char aes_key[AES_KEY_SIZE];
    memcpy(aes_key, sha256_key, AES_KEY_SIZE);

    log_hex("aes_open: aes_key: %s", aes_key, AES_KEY_SIZE);
...
static int64_t aes_seek( URLContext *h, int64_t pos, int whence) {
    ...
    switch (whence) {
		case SEEK_SET:
		    LOGI(3, "aes_seek: pos: %d, SEEK_SET", pos);
			// The offset is set to offset bytes.
			c->reading_position = pos;
			break;

		case SEEK_CUR:
		    LOGI(3, "aes_seek: pos: %d, SEEK_CUR", pos);
		...
		case AVSEEK_SIZE:
			...
			if (c->stream_end >= 0) {
				LOGI(3, "aes_seek: already_measured_size: %d", c->stream_end);
				return c->stream_end;
			}
			c->stream_end = ffurl_seek(c->hd, 0, AVSEEK_SIZE);
			LOGI(3, "aes_seek: measured_size: %d", c->stream_end);
			return c->stream_end;
		...
		case SEEK_END:
			...
			if (c->stream_end < 0) {
				c->stream_end = ffurl_seek(c->hd, 0, AVSEEK_SIZE);
				if (c->stream_end < 0) {
					LOGE(2, "aes_seek: could not measure size, error: %d", c->stream_end);
					return c->stream_end;
				}
			}
			LOGI(3, "aes_seek: measured_size: %d", c->stream_end);
	...
	LOGI(3, "aes_seek: reading_position: %d", c->reading_position);

	c->read_start_point = c->read_end_point = (c->reading_position / BUFFER_SIZE) * BUFFER_SIZE;
	LOGI(3, "aes_seek: read_start_point: %d", c->read_start_point);

    int64_t ret = ffurl_seek(c->hd, c->read_start_point, whence);
    LOGI(3, "aes_seek: return: %d", ret);
	if (ret < 0) {
		LOGE(1, "aes_seek: seeking error: %d", ret);
		return ret;
	}
	if (ret != c->read_start_point) {
		LOGE(1, "aes_seek: seeking fatal error: unkonwon state");
		return -2;
	}
...
static int aes_read(URLContext *h, uint8_t *buf, int size)
{
    ...
    while (buf_left > 0 && !end) {
    	LOGI(3, "aes_read loop, read_position: %d, buf_left: %d", c->reading_position, buf_left);
    	...
    	while (c->reading_position >= c->read_end_point && !end) {
    		LOGI(3, "aes_read read loop: current read_end_point %d", c->read_end_point);
    		int position = c->read_end_point;
    		int n = ffurl_read(c->hd, c->read_buff, BUFFER_SIZE);
    		...
    		*(int *)&c->iv[0] = (int)(c->read_start_point >> 9);
    		...
    		aes_crypt_cbc(&c->aes, AES_DECRYPT, n, c->iv, c->read_buff, c->decoded_buff);
    		static char print_buff[BUFFER_SIZE*2+1];
    		LOGI(3, "aes_read enc: position: %d", position);
    		log_hex("aes_read enc: encoded: %s", c->read_buff, n);
    		log_hex("aes_read enc: decoded: %s", c->decoded_buff, n);
    	}
